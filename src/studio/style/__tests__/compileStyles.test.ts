...
describe('compileStyles', () => {
  ...
  it('compiles per-side padding/margin tokens', () => {
    const styles: NodeStyles = { base: { pt: '4', pr: '2', pb: '4', pl: '2', mt: '8', ml: '0' } };
    expect(compileStyles(styles)).toBe('pt-4 pr-2 pb-4 pl-2 mt-8 ml-0');
  });

  it('prefixes per-side spacing across breakpoints', () => {
    const styles: NodeStyles = { base: { pt: '4' }, md: { pt: '8' } };
    expect(compileStyles(styles)).toBe('pt-4 md:pt-8');
  });

  it('prefixes per-side spacing in interaction states', () => {
    const styles: NodeStyles = { base: { mb: '2' }, states: { hover: { mb: '4' } } };
    expect(compileStyles(styles)).toBe('mb-2 hover:mb-4');
  });
...
describe('getSafelist', () => {
  ...
  it('includes per-side spacing classes and their variants', () => {
    const safelist = getSafelist();
    for (const cls of ['pt-4', 'pr-2', 'pb-8', 'pl-0', 'mt-4', 'mr-2', 'mb-8', 'ml-0']) {
      expect(safelist).toContain(cls);
    }
    // responsive + state prefixes are generated for per-side classes too
    expect(safelist).toContain('md:pt-8');
    expect(safelist).toContain('hover:mb-4');
  });
});
