...
const hue = (name: string, label: string, hex: string[]): PaletteHue => ({
  name,
  label,
  shades: Object.fromEntries(TAILWIND_SHADES.map((s, i) => [s, hex[i]])) as Record<TailwindShade, string>,
});

export const TAILWIND_PALETTE: PaletteHue[] = [
...
/** Fast lookup: hue name → hue. */
export const PALETTE_BY_NAME: Record<string, PaletteHue> = Object.fromEntries(
  TAILWIND_PALETTE.map((h) => [h.name, h])
);

/**
 * Preview color for a palette swatch: the live Tailwind v4 theme variable when
 * the host registered one, otherwise the static default hex.
 */
export const tailwindSwatch = (hueName: string, shade: TailwindShade): string => {
  const hex = PALETTE_BY_NAME[hueName]?.shades[shade] ?? '#000000';
  return `var(--color-${hueName}-${shade}, ${hex})`;
};

/** Parse a palette token (`red-500`) → { hue, shade }, or null when not one. */
export const parsePaletteToken = (
  value: string
): { hue: PaletteHue; shade: TailwindShade } | null => {
  const idx = value.lastIndexOf('-');
  if (idx <= 0) return null;
  const hueName = value.slice(0, idx);
  const shade = value.slice(idx + 1) as TailwindShade;
