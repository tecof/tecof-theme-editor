...
| Sol panel & blok paleti | `studio/panels/LeftPanel.tsx`, `TecofStudio.tsx` (`.tecof-left-panel-wrap`) | Sol panel kapanınca **tamamen** kapanır (solda rail yok; açma TopBar toggle'ında): hep mount, genişlik 0↔280 yumuşak geçiş + gecikmeli `visibility` (odak/tab dışı). Blok kategorileri **accordion** (başlık = sayaç + dönen chevron; `grid-template-rows 0fr↔1fr` animasyonu); kapalı kategoriler `tecof:blocks:collapsed-cats:v1` localStorage'ında kalıcı, arama aktifken accordion yok sayılır. Topbar orta kontrolleri tek yükseklik: `--tecof-topbar-ctrl-h` (28px) — viewports/vpwidth/mode-toggle/dil/ikon butonlar |
| Drawer kabuğu | `studio/ui/StudioDrawer.tsx`, `ConfirmDrawer.tsx` | Editördeki TEK pencere kabuğu (Radix Dialog **`modal={false}`**: body'ye `pointer-events:none` yazılmaz, odak tuzağı kurulmaz → drawer içinden açılan renk/ikon/CMS popover'ları tıklanabilir ve yazılabilir kalır; odak elle kaba çekilir). `size` sm/md/lg/xl, `tone` default/primary/danger, `busy`, `footer`, `headerActions`, `hideClose`, `elevated` (üst üste açılışta karartma bandı 100002/100003), `inert` (üstünde onay açıkken alt kartı tıklama+odak dışına alır). Kapatma ESC / karartma / X — **sürükleyerek kapatma yok** (vaul'dan çıkıldı). Global kısayol koruması `isStudioDrawerOpen()` (`data-state="open"` şartıyla; kapanış animasyonu ölü bölge yaratmaz) |
| Onay/uyarı kuyruğu | `studio/ui/dialogStore.ts`, `DialogHost.tsx` | zustand kuyruk: `confirm(): Promise<boolean>` / `alert(): Promise<void>`; hook dışı kullanım için `studioDialog`. `DialogHost` TecofStudio'da bir kez mount edilir, sıradakini `ConfirmDrawer` ile çizer; dış tıklama/ESC → confirm `false`. **`window.confirm`/`alert` kalmadı** (src/vendor hariç) |
| Modal → drawer dönüşümü | `panels/NodeSettingsModal`, `panels/HelpModal`, `style/StyleSyncModal`, `ai/AiSectionModal`, `panels/AddSectionModal`, `fields/PickerDrawer.tsx` | Altı pencere aynı kabuğa alındı (uiStore bayrakları ve iç sınıf adları KORUNDU, yalnız dış kabuk değişti). `PickerDrawer` e-ticaret `PickerShell`'i ile `ExternalField` portal panelini birleştirir. Dönüştürülmeyenler: CommandPalette, popover'lar, MediaDrawer/LinkPickerDrawer (hâlâ vaul + modal) |
| Görsel odak noktası | `utils/focalPoint.ts`, `components/TecofPicture.tsx`, `fields/FocalPointDrawer.tsx`, `fields/UploadField.impl.tsx` | `UploadedFile.focalPoint` `{x,y}` yüzde (50/50 = merkez), dosya nesnesinin İÇİNDE saklanır (backend `draftData` Mixed — şema değişikliği yok). TecofPicture img/video style'ına `object-position` yazar; çağıranın `imgStyle.objectPosition`'ı kazanır, odak yoksa/merkezdeyse DOM çıktısı değişmez. Drawer: tıkla/sürükle + ok tuşları (Shift ×10), 16:9/1:1/9:16 canlı kırpma önizlemeleri, "Merkeze al"; CDN dosyası, harici stok URL'i ve SVG destekli. Tile'da hover düğmesi + merkez dışı rozeti |
| Powered by bandı | `components/poweredBy/{resolve,usePoweredBy,PoweredBy}.ts(x)` | `<PoweredBy>` + `usePoweredBy`: saf çözümleyiciler `resolve.ts` (`pickLocale` prop → Studio dili → `<html lang>` → `defaultLanguage` → ilk; `pickMode` prop → `.dark` → class stratejisi → `prefers-color-scheme`; `resolveBand`, `isBandVisible`, `buildBandCss`), kanca `usePoweredBy` (SWR anahtarı `tecof:merchant-info:<apiUrl>:<secretKey>`, 10 dk dedupe, `initialData`+`defaultLanguage` SSR kaynağı, `revalidateOnMount = initialData === undefined`, `.dark`/OS canlı izleme `useSyncExternalStore`), bileşen her zaman İKİ varyant basar (tek modda pasif `hidden` → innerHTML yeniden basılmaz, admin JS dinleyicileri yaşar), JS sürüm başına bir kez `ensureBandScript`, `<style>` örnek başına ve `[data-powered-by]{display:none}` içerir. `useLanguages` ayrı Map önbelleği (SWR değil) |
| ColorField v2 | `fields/ColorField.tsx`, `fields/color/{colorMath,ColorPicker,useThemePalette,recentColors,sliderKeys}.ts(x)`, `studio/theme/colorKeys.ts` | Değer sözleşmesi `''`/`#rrggbb`/`#rrggbbaa`/`var(--theme-color-*)` (`classifyValue`/`normalizeValue`); popover `document.body`'ye portallanır, pointer capture ile sürükleme, roving tabindex nokta satırları, kanal girdileri (Esc yalnız taslak varken yutulur), yazılırken kısa hex hata değil (`isColorDraftInProgress`). `useThemePalette` Studio'da açık/koyu/`current` çözer; tema noktası kopya modunda kanonik AÇIK değeri (`copyCss`) yazar, `themeVars` ile `var()` bağlar; "Bağı kopar" `themeEntry.light` ya da host belgesinin CSS değişkeni, çözüm yoksa düğme yok. `THEME_COLOR_KEYS` tek kaynak (ThemeEditor satırları, `background` satırında kontrast kapalı) |
| Dil araçları (ortak) | `fields/languageTools.ts`, `fields/LanguageToolsBar.tsx` | Saf yardımcılar: `normalizeLocalizedValues`, `fillLanguages` (kaynak = AKTİF sekme, `onlyEmpty` ile yalnız boş diller — LinkField/LocalizedUpload), `translateLanguages` (`TranslateFn` enjekte), `isEmptyText`/`isEmptyHtml` (medya etiketi dolu sayılır)/`isEmptyLink`, `LANGUAGE_TOOL_MESSAGES`. Çubuk: Hızlı Doldur + (isteğe bağlı) Çevir + durum mesajı (`role=status|alert`, `aria-live`, `aria-busy`); LanguageField, EditorField, LinkField, LocalizedUploadField kablolar |
| Çok dilli medya alanı | `utils/localizedUpload.ts`, `fields/LocalizedUploadField.tsx` | `createLocalizedUploadField` (`_fieldType: 'localized-upload'`) — değer `[{code, value: UploadedFile[]}]`; gövde mevcut lazy `UploadField` (`key={activeTab}` ile dil başına FilePond/drawer/odak sıfırlanır), dil üçlüsü LinkField ile aynı (`useLanguages` + `useActiveLanguage`). Tembel göç (mount'ta emit yok; düz `UploadedFile[]` varsayılan dil altında), bilinmeyen kodlar korunur, Hızlı Doldur yalnız BOŞ dillere sığ klon, Çevir yok. Tema tarafı `resolveLocalizedUpload`/`resolveLocalizedUploadFile`: locale → varsayılan → ilk dolu → `[]`; core `getL` boş diziyi dolu saydığı için ayrı çözümleyici. Saf mantık testli (`utils/__tests__/localizedUpload.test.ts`) |
| Panel bağlantıları | `utils/panelLinks.ts`, `fields/PanelLink.tsx`, `studio/context.tsx` | `TecofEditorProps.panelUrl` (varsayılan `https://app.tecof.com`) → `StudioContext`; `useStudioOptional()` provider yokken null döner (alanlar Studio dışında da render edilir). Yollar tek kaynak `PANEL_PATHS` + `buildPanelUrl` (çift slash temizler, boş tabanda göreli yol). E-ticaret seçicileri (8 kaynak `panelPath`/`panelNewPath`/`panelLabel`), LinkPicker'ın altı boş durumu ve CmsCollectionField açılırı bağlandı; yeni sekme + `rel="noopener noreferrer"`, arama kutusu doluyken çizilmez |
| Sayfa şablonu onayı | `panels/PageTemplateConfirmDrawer.tsx`, `pageTemplatePreview.ts`, `PageTemplateMiniPreview.tsx` | `PageTemplate.previewUrl` varsa 1280px referansta render edilip ResizeObserver ölçümüyle `transform: scale()` ile ~60vh kutuya sığdırılan iframe (iskelet + `loading="lazy"` + sandbox + `pointer-events:none`), yoksa bölümlerin `LiveBlockPreview` yığını; bölüm çipleri + "tek Geri Al" notu. Sol panel ve AddSection drawer'ı aynı drawer'ı kullanır, onaydan sonra mevcut ekleme yolu (tek commit) yürür. Ölçek/etiket hesapları saf modülde ve testli. Sol panel kartı: thumbnail → minik canlı yığın → ikon; `.tecof-block-cat-items > *` taşma düzeltmesi. AddSection grup başlıkları akordeon (`tecof:add-section:collapsed:v1`, varsayılan AÇIK, arama sırasında hepsi açık) |
